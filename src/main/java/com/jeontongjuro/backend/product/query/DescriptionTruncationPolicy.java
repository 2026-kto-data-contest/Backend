 ...
 * </ol>
 * <p>
 * ★"문장 끝" = 마침표({@code '.'}) 바로 뒤에 공백이 오거나, 마침표가 문자열 맨 끝인 위치. 그 마침표까지(포함)
 * 를 남기고 뒤를 버린다. 문장 끝이 하나도 없으면 null(미노출)을 반환한다.
 * <ul>
 *   <li>소수점은 자연 회피된다 — {@code "5.8"}의 마침표는 뒤가 숫자라 공백도 문자열 끝도 아니어서 무시된다
 *       (예: {@code "…막걸리. 순수령 5.8도와"} → {@code "…막걸리."}).</li>
 *   <li>맨 끝 마침표 케이스(이미 완결된 문장이 78·79자인 경우)는 되돌림 결과가 원문과 같다.</li>
 * </ul>
 * <p>
...
public final class DescriptionTruncationPolicy {
    ...
    /**
     * 마지막 "문장 끝" 마침표의 인덱스. 뒤에서 앞으로 훑어 첫 번째로 만나는, 뒤가 공백이거나 문자열 끝인
     * 마침표를 반환한다. 없으면 -1.
     */
